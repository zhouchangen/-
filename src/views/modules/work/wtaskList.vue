        ...
        <template slot-scope="scope">
          <el-popover class="popContainer" width="650" trigger="click">
            <el-form :inline="true" :model="strForm" @keyup.enter.native="getDataList(1)">
              <el-form-item label="状态：">
                <el-radio-group v-model="strForm.status" @change="btnChange($event)">
                  <el-radio
...
export default {
  ...
  methods: {
    ...
    getDataList(num) {
      ...
      this.$http({
        url: this.$http.adornUrl("/workbench/task/myList?finish=" + false + "&type=" + 1),
        method: "get",
        params: this.$http.adornParams({
