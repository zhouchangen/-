<template>
  <textarea :id='id' :value='value' ref="ggg"></textarea>
</template>
  ...
  `,
        ...
        images_upload_handler: function(blobInfo, success, failure) {
          ...
          function uploadPic() { // 发送请求
            ...
            let url = _this.$http.adornUrl(`/sys/oss/upload?token=${_this.$cookie.get('token')}`)
            // let url = `http://192.168.11.238/dev/car-dealer-admin/sys/oss/upload?token=${_this.$cookie.get('token')}`
